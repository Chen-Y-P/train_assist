...
public class WebUserSqlProvider extends BaseSqlProvider {
	...
	public String searchByConditions(final WebUser webUser) {
		...
		if (StringUtils.isNotBlank(webUser.getUserName())) {
			...
			sql.append(" AND A.USERNAME LIKE CONCAT('%', '").append(webUser.getUserName()).append("', '%')");
		}
		if (StringUtils.isNotBlank(webUser.getPostName())) {
			sql.append(" AND A.POSTNAME LIKE CONCAT('%', '").append(webUser.getPostName()).append("', '%')");
		}
		if (StringUtils.isNotBlank(webUser.getDeptName())) {
			sql.append(" AND A.DEPTNAME LIKE CONCAT('%', '").append(webUser.getDeptName()).append("', '%')");
		}
		if (StringUtils.isNotBlank(webUser.getAliases())) {
			sql.append(" AND A.ALIASES LIKE CONCAT('%', '").append(webUser.getAliases()).append("', '%')");
		}
		return sql.toString();
	}
